# 3.4 is just for testing out the effect of multiprocessing (and possible other minor tweaks)
...
startdate = datetime.datetime.strptime("2023-01-1",'%Y-%m-%d').date()
enddate = datetime.date.today()
enddate = datetime.datetime.strptime("2023-09-1",'%Y-%m-%d').date()
...
#resultspartialdataframes = [] # now moved inside the 'if main...'
...
def TradeFunctionDaily(stock): # columns in DB Daily -> check excel file for column definitions
    ...
    if len(df_tradesignals) > 0:
        ...
        try:
            ...
            # return the partial dataframe (temporary storage)
...
def resultsdataframe_creator():
    ...
    # the 'resultsdataframe' is created in the 'if main' block
    return resultsdataframe.reindex(indexlist, axis=1)
...
def start_backtest():
    ...
    with multiprocessing.Pool(processes=16) as pool:
        results = pool.map(TradeFunctionDaily, ticker_list)

    return results


if __name__ == '__main__':
    print(f"Starting a a new backtest for {len(ticker_list)} stocks")
    resultspartialdataframes = start_backtest()

    resultsdataframe = resultsdataframe_creator()
    logdataframe = logdataframe_creator()
    results_displayer()
    logger_to_file()
